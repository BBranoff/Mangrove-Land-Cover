...
# this is the "mods.csv" file on GitHub
...
##  This is the "PRMANGROVE_HOBOZones.zip" file on GitHub
HOBO_zone <- readOGR("PRMangrovesCombined_HOBOS.shp")
##  This is the "HOBOS.zip" file on GitHub
HOBO_loc <- readOGR("HOBOS.shp")
## Elevation rasters are too big for GitHub and should be aquired independently
Elev <- raster("San_Juan_DEM_1602/DEM.tif")  #DEMs are referenced to the vertical tidal datum of Mean High Water (MHW)
Elev <- crop(Elev, HOBO_zone)
#Elev <- mask(Elev, HOBO_zone)
ElevPon <- raster("PonceMosaicNOAASLR_DEM.tif") 
ElevPon <- crop(ElevPon, HOBO_zone)
#ElevPon <- mask(ElevPon, HOBO_zone)
ElevPon <- aggregate(ElevPon,fact = res(Elev)/res(ElevPon),fun="mean",na.rm=T)
Mods$time <- as.POSIXct(Mods$time,format="%m/%d/%Y %H:%M")  ##  Set the time column of the Mods dataframe to a POSIXct data type
for (i in 1:length(HOBO_zone)){  ### For each location within the mangrove (each piezometer), we will create a dataframe of waterlevels at random locations
  if (HOBO_zone$HOBO[i] %in% c("PONMAX","PONMIN","PONMID")){  ## If the mangroves are in Ponce and include "PON" in their name, we use the Ponce DEM
    rec <- crop(ElevPon, extent(HOBO_zone[HOBO_zone$HOBO==HOBO_zone$HOBO[i],]))  ## rec will be the dem used for each location
  }else{  ## If the location is not in Ponce, we will use the San Juan DEM, which includes Levittown
    rec <- crop(Elev, extent(HOBO_zone[HOBO_zone$HOBO==HOBO_zone$HOBO[i],]))
  }
  ...
  elev <- extract(rec,HOBO_loc[HOBO_loc$HOBOS==HOBO_zone$HOBO[i],])  ## extract the elevation of the piezometer
  for (j in 1:nrow(points)){  ## for each of the random points, calculate the water level at each time step
    df[,j] <- Mods[,gsub("Pred","mod",HOBO_zone$Preds[i])]+(elev-points[j,3])  ## the water level is the water level of the piezometer plus the elevation of the piezometer minus the elevation fo the point
  }
  ...
  df$loc <- as.character(HOBO_zone$HOBO[i]) ## save the location name to the dataframe
  ...
  listofpoints_100[[i]]$loc <- as.character(HOBO_zone$HOBO[i])
