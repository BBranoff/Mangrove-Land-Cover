####  This is a collection of subroutines meant to calculate surrounding land cover in previously established random points within mangroves
####  Required Inputs are:
...
###               an attribute with the name "loc". 
###       A list of dataframes containing the random points within the mangroves. This was created in the "Mangrove Points" subroutine
###       The various spatial datasets we will use to calculate land cover:
##          A land cover raster (LCType), this is a NOAA 2010 Land Cover raster for the coastal US and Puerto Rico: Office for Coastal Management, 2017. C-CAP Land Cover, Puerto Rico, 2010. Charleston, SC. https://coast.noaa.gov/dataviewer/#/imagery/search/where:ID=6201%0A
##          A mangrove cover raster (Mangr), this is a data set from: Hamilton, S.; and Casey, D., 2016. Creation of a high spatiotemporal resolution global database of continuous mangrove forest cover for the 21st Century (CGMFC-21). Glob. Ecol. Biogeogr. 25.6, 729-738.
##          A network of roads and higways within the study area (St and Highways) : U.S. Census Bureau, 2015. 2015 TIGER/Line Shapefiles roads.
##          A list of total population numbers for census blocks (Pop): U.S. Census Bureau, 2010. Total population at census block level.
##          A shapefile of census blocks (Census) : U.S. Census Bureau, 2010. Total population at census block level.
LandCoverStats <- function(){
  pdata <- matrix(nrow = 0,ncol = 14)  ## create an empty dataset. This will hold the landcover data for each point
  ##  the variables we will calculate are the surrounding mangrove coverage, urban area, green and blue area, crop area, population density, and street and highway length
  colnames(pdata) <- c("y","x","elev","HOBO","CellCount","MangCellCount","Area","MangArea","UrbanArea","GBArea","CropArea","PopDens","StLength","HwyLength")
  
  # create progress bar to keep track of the progress
  # First calculate the total number of iterations that will be completed. Because we will do 100 points for each location, it is 100 times the number of locations
  total <- 100*length(listofpoints)
  pb <- txtProgressBar(min = 1, max = total, style = 3)  ## initialize the progress bar
  c <- 1  ## set the counter for the progress bar to 1
  
  for (i in 1:length(listofpoints)){  ## for each location in the mangroves, or each set of points, we will calculate land cover at the random points
    for (points in listofpoints){   ## cycle through the list of points
      colnames(points) <- c("x","y","elev","HOBO")  ## set the names of the points data frame so that we can convert it to a spatial points data frame
      points <- SpatialPointsDataFrame(points[, c("x", "y")], 
                                       data.frame(elev = points$elev,
                                                  HOBO = points$HOBO), 
                                       proj4string=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84"))
      for (n in 1:length(points)){  ## for each point, run the landcover subroutines below and calculate the surrounding land cover
        BufferStats(points[n,], LCType, Pop, Mangr,St,Highways,Census)  ## Run the primary subroutine "Buffer Stats", which creates sampling buffers and samples within them
        write.csv(pdata,"LandUse.csv",row.names=F)  ## save the land cover data frame after each point
        Sys.sleep(0.1)  ## sleep for a tenth of a second to allow the progress bar to operate
        setTxtProgressBar(pb, c)  # advance the progress bar
        c <- c+1  ## advance the counter
        removeTmpFiles(h=0)  ## remove any temp files from the memory
      }
    }
    
  }
  remove(LCType,Mangr,Pop,St,Elev,ElevPon,points,Streets,rec,r,pb,locs,df,CellsMang1,Count1,Crop1,elev,GB1,i,j,l,
         loc,Mang1,minmax,PopDens1,n,Urban1,StLength,t)
}


BufferStats <- function(p,L,P,M,St,High,Cen){
  ## This routine calculates surrounding land cover at a given spatial point. 
  ## Inputs are provided by the "LandCoverStats" function and but they are:
